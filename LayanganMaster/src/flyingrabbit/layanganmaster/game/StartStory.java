...
public class StartStory extends SimpleBaseGameActivity {
	...
	private ITextureRegion mBackgroundTextureRegion, mGirl, mBoy1, mBoy2,
			mLove, mKuda, mBoy3, mIstana, mKuda2, mPenjaga, mBoy4, mBoy5,
			mLamp, mAwan, mLayangan, mSurat, mKirim;
	int cameraWidth;
	int cameraHeight;
	final Scene scene = new Scene();
	private BitmapTextureAtlas mBitmapTextureAtlas;
	Sprite background, girl, boy1, boy2, love, kuda, boy3, kuda2, istana,
			penjaga, boy4, boy5, lamp, awan, layangan, surat, kirim;
	Music music;
	float scale;
	...
	protected void onCreateResources() {
		...
		try {
			music = MusicFactory.createMusicFromAsset(
					...
					"msc/Benteng_Master-Opening.mp3");
					...
					public void onTimePassed(TimerHandler pTimerHandler) {
						if (i == 0) {
							...
							kuda = new Sprite(0, 0, mKuda,
									getVertexBufferObjectManager());
							kuda.setScale(scale);
							kuda.setPosition(-kuda.getWidthScaled() / 2,
									kuda.getHeightScaled() / 2
											+ player.paddingY);
							scene.attachChild(kuda);
							MoveXModifier lari = new MoveXModifier(5f, -(kuda
									.getWidthScaled() / 2), cameraWidth / 3
									+ (kuda.getWidthScaled() / 2));
							...
							i++;
							MoveXModifier pergi = new MoveXModifier(3,
									cameraWidth / 3
											+ (kuda.getWidthScaled() / 2),
									cameraWidth + (kuda.getWidthScaled()));
							...
							istana = new Sprite(bgPositionX, bgPositionY,
									mIstana, getVertexBufferObjectManager());
							...
							i++;
							kuda2 = new Sprite(cameraWidth, bgPositionY,
									mKuda2, getVertexBufferObjectManager());
							kuda2.setScale(scale);
							scene.attachChild(kuda2);
							MoveXModifier lari2 = new MoveXModifier(5,
							...
							penjaga = new Sprite(bgPositionX, bgPositionY,
									mPenjaga, getVertexBufferObjectManager());
							...
							layangan = new Sprite(bgPositionX, bgPositionY,
									mLayangan, getVertexBufferObjectManager());
							layangan.setScale(scale);
							scene.attachChild(layangan);
							i++;
						} else if (i == 19) {
							surat = new Sprite(bgPositionX, bgPositionY,
									mSurat, getVertexBufferObjectManager());
							surat.setScale(scale);
							scene.attachChild(surat);
							i++;
						} else if (i == 20) {
							kirim = new Sprite(bgPositionX, bgPositionY,
									mKirim, getVertexBufferObjectManager());
